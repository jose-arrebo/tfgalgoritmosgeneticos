...
def fitness1(sol,l,c,i,n): #l: una lista de tuplas (peso,valor); i: iteracion
    sol_bits = trans_bits(sol, l, c)
    result = 0
    ptotal = 0
    ben = 0
    for j in range(len(sol_bits)):
        (peso,beneficio) = l[j]
        ben = ben + beneficio
        if (sol_bits[j] == 1):
            result = result + beneficio
...
#estudiom_valorabs(terna)
#estudiom_valorabs(terna11)
#estudiom_valorabs(terna111)
#rendimiento_natmochila(terna2, 100, 50)
rendimiento_natmochila(terna22, 100, 50)
#c, pesos, benef = terna2[6]
#l = list(zip(pesos,benef))
#alg_gen_MochilaNat(100, 50, l, c)